...
BnPtr bn_from(std::span<const std::uint8_t> bytes) { return {BN_bin2bn(bytes.data(), bytes.size(), nullptr), BN_free}; }

Bytes padded(const BIGNUM* value) {
    Bytes result(kGroupBytes);
    return BN_bn2binpad(value, result.data(), result.size()) == static_cast<int>(result.size()) ? result : Bytes{};
}
...
Bytes sha512(const std::initializer_list<std::span<const std::uint8_t>> parts) {
    ...
    if (!context || EVP_DigestInit_ex(context.get(), EVP_sha512(), nullptr) != 1) return {};
    for (const auto part : parts) {
        if (EVP_DigestUpdate(context.get(), part.data(), part.size()) != 1) return {};
    }
    if (EVP_DigestFinal_ex(context.get(), digest.data(), &size) != 1) return {};
    return {digest.begin(), digest.begin() + static_cast<std::ptrdiff_t>(size)};
}

bool equal(std::span<const std::uint8_t> left, std::span<const std::uint8_t> right) {
    return left.size() == right.size() && CRYPTO_memcmp(left.data(), right.data(), left.size()) == 0;
}

}  // namespace

bool SrpClient::process_challenge(const std::span<const std::uint8_t> salt,
                                  const std::span<const std::uint8_t> server_public_key) {
    public_key_.clear();
    client_proof_.clear();
    session_key_.clear();
    if (salt.empty() || server_public_key.size() != kGroupBytes) return false;
    BIGNUM* raw_prime = nullptr;
    if (BN_hex2bn(&raw_prime, kPrimeHex) == 0) return false;
    ...
    if (!prime || !generator || !server || !context || BN_set_word(generator.get(), 5) != 1) return false;
    auto remainder = bn_new();
    if (!remainder || BN_mod(remainder.get(), server.get(), prime.get(), context.get()) != 1 || BN_is_zero(remainder.get())) return false;

    std::array<std::uint8_t, 32> private_random{};
    if (RAND_bytes(private_random.data(), private_random.size()) != 1) return false;
    auto private_key = bn_from(private_random);
    auto client = bn_new();
    if (!private_key || !client || BN_mod_exp(client.get(), generator.get(), private_key.get(), prime.get(), context.get()) != 1) return false;
    ...
    const auto user_password = sha512({std::span(reinterpret_cast<const std::uint8_t*>(username.data()), username.size()),
                                      colon,
                                      std::span(reinterpret_cast<const std::uint8_t*>(password.data()), password.size())});
    ...
    if (!multiplier || !x || !verifier || !u || BN_mod_exp(verifier.get(), generator.get(), x.get(), prime.get(), context.get()) != 1) return false;
    ...
    if (!multiplier_verifier || !base || !ux || !exponent || !shared ||
        BN_mod_mul(multiplier_verifier.get(), multiplier.get(), verifier.get(), prime.get(), context.get()) != 1 ||
        BN_mod_sub(base.get(), server.get(), multiplier_verifier.get(), prime.get(), context.get()) != 1 ||
        BN_mod_mul(ux.get(), u.get(), x.get(), prime.get(), context.get()) != 1 ||
        BN_mod_add(exponent.get(), private_key.get(), ux.get(), prime.get(), context.get()) != 1 ||
        BN_mod_exp(shared.get(), base.get(), exponent.get(), prime.get(), context.get()) != 1) return false;
    ...
    for (std::size_t index = 0; index < xor_hash.size(); ++index) xor_hash[index] = h_prime[index] ^ h_generator[index];
    const auto h_username = sha512({std::span(reinterpret_cast<const std::uint8_t*>(username.data()), username.size())});
    client_proof_ = sha512({xor_hash, h_username, salt, public_key_, server_public_key, session_key_});
    return client_proof_.size() == 64;
}

bool SrpClient::verify_server(const std::span<const std::uint8_t> server_proof) const {
    if (public_key_.empty() || client_proof_.empty() || session_key_.empty()) return false;
    return equal(server_proof, sha512({public_key_, client_proof_, session_key_}));
}

...
public:
  ...
  bool start(std::string *error) {
    ...
    const auto result = libusb_hotplug_register_callback(
        usb_context_, LIBUSB_HOTPLUG_EVENT_DEVICE_LEFT, LIBUSB_HOTPLUG_NO_FLAGS,
        LIBUSB_HOTPLUG_MATCH_ANY, LIBUSB_HOTPLUG_MATCH_ANY,
        LIBUSB_HOTPLUG_MATCH_ANY, &Impl::device_left, this,
        &departure_callback_);
  ...
  void stop() {
    {
      ...
      work_guard_.reset();
      io_service_.stop();
      if (departure_callback_ != LIBUSB_HOTPLUG_MATCH_ANY)
        libusb_hotplug_deregister_callback(usb_context_, departure_callback_);
      departure_callback_ = LIBUSB_HOTPLUG_MATCH_ANY;
    }
...
private:
  ...
  static int device_left(libusb_context *, libusb_device *device,
                         libusb_hotplug_event, void *user_data) {
    auto *self = static_cast<Impl *>(user_data);
    if (!self->active_ || self->stopping_ ||
        libusb_get_bus_number(device) != self->active_bus_ ||
  ...
  libusb_hotplug_callback_handle departure_callback_{LIBUSB_HOTPLUG_MATCH_ANY};
  ...
  std::uint8_t active_bus_{};
  std::uint8_t active_address_{};

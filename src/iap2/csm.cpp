...
std::vector<std::uint8_t> encode(const std::uint16_t id, const std::span<const std::uint8_t> payload) {
...
std::vector<std::uint8_t> encode_bytes_parameter(const std::uint16_t id, const std::uint16_t parameter_id,
...
std::optional<std::vector<std::uint8_t>> first_bytes_parameter(const std::span<const std::uint8_t> payload,
    ...
    while (offset + 4 <= payload.size()) {
        ...
        if (id == parameter_id) {
            return std::vector<std::uint8_t>(payload.begin() + static_cast<std::ptrdiff_t>(offset + 4),
...
bool verify_ecdsa_sha256(const std::span<const std::uint8_t> challenge,
    ...
    X509* certificate = d2i_X509(nullptr, &certificate_ptr, static_cast<long>(certificate_der.size()));
    ...
    const auto initialized = EVP_DigestVerifyInit(context, nullptr, EVP_sha256(), nullptr, key) == 1;
    const auto verified = initialized &&
                          EVP_DigestVerify(context, signature.data(), signature.size(), challenge.data(),
                                           challenge.size()) == 1;
...
std::optional<Message> Decoder::next() {
    while (buffer_.size() >= 6) {
        ...
        Message result{read_u16(buffer_, 4), {buffer_.begin() + 6, buffer_.begin() + length}};

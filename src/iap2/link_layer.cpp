...
std::uint16_t read_u16(std::span<const std::uint8_t> bytes, const std::size_t offset) {
    return static_cast<std::uint16_t>((static_cast<std::uint16_t>(bytes[offset]) << 8U) |
                                      bytes[offset + 1]);
}

void write_u16(std::span<std::uint8_t> bytes, const std::size_t offset, const std::uint16_t value) {
...
std::optional<Lsp> decode_lsp(const std::span<const std::uint8_t> bytes) {
    ...
    for (std::size_t offset = 10; offset < bytes.size(); offset += 3) {
        result.sessions.push_back({bytes[offset], bytes[offset + 1], bytes[offset + 2]});
    }
    return result;
}
...
std::optional<Header> decode_header(const std::span<const std::uint8_t, 9> bytes) {
...
PhoneLink::PhoneLink(SendFunction send, LogFunction log, ControlDataFunction control_data)
    : send_(std::move(send)), log_(std::move(log)), control_data_(std::move(control_data)) {}
...
void PhoneLink::receive(const std::span<const std::uint8_t> bytes,
    ...
    if (state_ == State::Detect) {
        const auto marker = std::search(receive_buffer_.begin(), receive_buffer_.end(),
                                        kMarker.begin(), kMarker.end());
        if (marker == receive_buffer_.end()) {
            if (receive_buffer_.size() > kMarker.size() - 1) {
                receive_buffer_.erase(receive_buffer_.begin(),
                                      receive_buffer_.end() - static_cast<std::ptrdiff_t>(kMarker.size() - 1));
            }
            return;
        }
        receive_buffer_.erase(receive_buffer_.begin(), marker + static_cast<std::ptrdiff_t>(kMarker.size()));
...
void PhoneLink::write_packet(const std::span<const std::uint8_t> payload, const std::uint8_t sequence,
                             const std::uint8_t control, const std::uint8_t session_id) {
    const auto header = encode_header({static_cast<std::uint16_t>(payload.empty() ? 9 : payload.size() + 10),
...
void PhoneLink::process_packets(const std::chrono::steady_clock::time_point now) {
    while (receive_buffer_.size() >= 9) {
        const auto start = std::find_if(receive_buffer_.begin(), receive_buffer_.end() - 1,
        ...
        std::copy_n(receive_buffer_.begin(), header_bytes.size(), header_bytes.begin());
        ...
        if (header->length > 9) {
            const auto payload_and_checksum = std::span(receive_buffer_).subspan(9, header->length - 9);
            if (!has_valid_checksum(payload_and_checksum)) {
                receive_buffer_.erase(receive_buffer_.begin(), receive_buffer_.begin() + header->length);
                continue;
            }
            payload.assign(payload_and_checksum.begin(), payload_and_checksum.end() - 1);
        }
        receive_buffer_.erase(receive_buffer_.begin(), receive_buffer_.begin() + header->length);
...
void PhoneLink::handle_packet(const Header& header, const std::span<const std::uint8_t> payload,
                              const std::chrono::steady_clock::time_point now) {

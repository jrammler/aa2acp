...
std::vector<std::uint8_t> attributes(const bool available,
                                     const std::string &identifier) {
  std::vector<std::uint8_t> output;
  const std::array<std::uint8_t, 1> availability{
...
std::string string_parameter(const csm::Message &message,
                             const std::uint16_t id) {
  const auto value = csm::first_bytes_parameter(message.payload, id);
  if (!value) {
    return {};
  }
  const auto end = value->empty() ? value->end()
                                  : value->end() - (value->back() == 0 ? 1 : 0);
  return {value->begin(), end};
}

std::uint32_t u32_parameter(const csm::Message &message,
                            const std::uint16_t id) {
  const auto value = csm::first_bytes_parameter(message.payload, id);
...
void CarPlayProbe::request_wifi_configuration(const bool enabled) {
  request_wifi_ = enabled;
}

void CarPlayProbe::set_wifi_join_handler(
...
void CarPlayProbe::handle(const csm::Message &message) {
  ...
  if (message.id == kCarPlayStartSession) {
    ...
    if (link_ == nullptr ||
        !link_->send_control(csm::encode(kRequestAccessoryWifiConfiguration))) {
  ...
  if (message.id == kAccessoryWifiConfiguration &&
      awaiting_wifi_configuration_) {
    const AccessoryWifiConfiguration configuration{
        string_parameter(message, 1), string_parameter(message, 2),
        u8_parameter(message, 3), u8_parameter(message, 4)};
    if (configuration.ssid.empty()) {
      fail("accessory Wi-Fi configuration has no SSID");
      return;
    }
    std::cout << "Wi-Fi: received SSID '" << configuration.ssid
              << "' (security " << static_cast<int>(configuration.security_type)
    ...
    if (wifi_join_handler_) {
      ...
      if (link_ == nullptr || !link_->send_control(csm::encode_bytes_parameter(
                                  kWirelessCarPlayUpdate, 0, joined))) {

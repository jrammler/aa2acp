...
void test_negotiation() {
  ...
  assert(writes[0] == std::vector<std::uint8_t>(acp::iap2::kMarker.begin(),
                                                acp::iap2::kMarker.end()));
  ...
  const auto syn_header =
      acp::iap2::encode_header({static_cast<std::uint16_t>(lsp.size() + 10),
                                acp::iap2::kControlSyn, 3, 100, 0});
  ...
  const auto ack =
      acp::iap2::encode_header({9, acp::iap2::kControlAck, 4, 100, 0});
...
void test_csm_codec() {
  ...
  assert(decoded.has_value());
  assert(decoded->id ==
         acp::iap2::csm::kRequestAuthenticationChallengeResponse);
  assert(decoded->payload.size() == 7);
  const auto parameter =
      acp::iap2::csm::first_bytes_parameter(decoded->payload, 0);

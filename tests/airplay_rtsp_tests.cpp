...
int main() {
    ...
    const auto request = acp::airplay::encode_request("POST", "/pair-setup", 1, request_body,
    ...
    const std::string raw = "RTSP/1.0 200 OK\r\nContent-Length: 3\r\nCSeq: 1\r\n\r\nabc";
    const auto parsed = acp::airplay::parse_response(
    ...
    const auto encoded = acp::airplay::encode_tlv8({{3, {large.begin(), large.end()}}, {6, {2}}});

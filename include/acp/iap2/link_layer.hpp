...
constexpr std::array<std::uint8_t, 6> kMarker{0xff, 0x55, 0x02, 0x00, 0xee, 0x10};
...
struct Lsp {
    ...
    std::vector<Session> sessions{{kControlSessionId, 0, 2}, {11, 2, 1}, {12, 1, 2}};
};
...
class PhoneLink {
  public:
    explicit PhoneLink(SendFunction send, LogFunction log = {}, ControlDataFunction control_data = {});
  ...
  private:
    ...
    void send_ack();
    void write_packet(std::span<const std::uint8_t> payload, std::uint8_t sequence,
                      std::uint8_t control, std::uint8_t session_id = 0);
    void process_packets(std::chrono::steady_clock::time_point now);
    void handle_packet(const Header& header, std::span<const std::uint8_t> payload,
                       std::chrono::steady_clock::time_point now);
